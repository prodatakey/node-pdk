...
export async function getSystemToken(session, id) {
  // This must conform to the token_set identity
  // see its use in session.js
  ...
  let id_token;
  const token_set = async () => {
    debug('getting system token from tokenset')
  ...
  token_set.refresh = async() => {
    debug('refreshing system token')
    id_token = (await session(`systems/${id}/token`, { method: 'POST' })).token;
    debug('refreshed system token')
  };
...
export async function getSystem(session, id) {
  return await session(`systems/${id}`);
}
